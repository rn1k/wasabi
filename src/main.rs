...
#[repr(C)]
#[derive(Debug)]
struct EfiGraphicsOutputModePixelInfo {
    version: u32,
...
const _: () = assert!(size_of::<EfiGraphicsOutputModePixelInfo>() == 36);
...
struct EfiGraphicsOutputProtocolMode<'a> {
    pub max_mode: u32,
    pub mode: u32,
    pub info: &'a EfiGraphicsOutputModePixelInfo,
...
fn efi_main(_image_handle: EfiHandle, efi_system_table: &EfiSystemTable) {
    ...
    for (i, c) in "ABCDEF".chars().enumerate() {
        let _ = draw_font_fg(&mut vram, i as i64 * 16 + 256, i as i64 * 16, 0xffffff, c);
    }
...
fn draw_font_fg<T: Bitmap>(buf: &mut T, x: i64, y: i64, color: u32, c: char) -> Result<()> {
...
fn draw_str_fg<T: Bitmap>(buf: &mut T, x: i64, y: i64, color: u32, s: &str) {
    for (i, c) in s.chars().enumerate() {
        let _ = draw_font_fg(buf, x + i as i64 * 8, y, color, c);
    }
}
